...
namespace AramBuddy.MainCore.Utility
{
    internal class ObjectsManager
    {
            ...
            get
            {
                return
                    EntityManager.Heroes.Allies.OrderByDescending(a => Misc.TeamTotal(a) - Misc.TeamTotal(a, true))
                                 ...
                                 .FirstOrDefault(
                                     a =>
                                         a.IsValidTarget() && (a.HealthPercent > 10) && !a.IsInShopRange() && !a.IsDead &&
                                         !a.IsZombie && !a.IsMe);
            }
        }
            ...
            get
            {
                return
                    HealthRelics.OrderBy(e => e.Distance(Player.Instance))
                                .FirstOrDefault(
                                    e =>
                                        e.IsValid && (e.Distance(Player.Instance) < 800) &&
                                        (e.CountEnemiesInRange(1000) < 1));
            }
        }
            ...
            get
            {
                return
                    EntityManager.MinionsAndMonsters.AlliedMinions.OrderByDescending(a => a.Distance(AllyNexues))
                                 .FirstOrDefault(
                                     m =>
                                         m.IsValidTarget() && m.IsValid && m.IsHPBarRendered && !m.IsDead &&
                                         (m.Health > 65)
            ...
            get
            {
                return
                    EntityManager.Turrets.Allies.OrderBy(t => t.Distance(Player.Instance))
                                 .FirstOrDefault(t => t.IsValidTarget() && !t.IsDead);
            }
        }
        ...
        public static Obj_AI_Turret EnemyTurret
        {
            ...
            {
                return
                    EntityManager.Turrets.Enemies.OrderBy(t => t.Distance(Player.Instance))
                                 .FirstOrDefault(t => t.IsValidTarget() && !t.IsDead);
            }
        }
            ...
            get
            {
                return
                    ObjectManager.Get<Obj_BarracksDampener>()
                                 .FirstOrDefault(i => i.IsEnemy && i.IsValidTarget() && !i.IsDead);
            }
        }

        public static Obj_HQ EnemyNexues
        {
            get { return ObjectManager.Get<Obj_HQ>().FirstOrDefault(i => i.IsEnemy); }
        }

        public static Obj_HQ AllyNexues
        {
            get { return ObjectManager.Get<Obj_HQ>().FirstOrDefault(i => i.IsAlly); }
        }
